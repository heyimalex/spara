...
// `workers` separate goroutines, calling it with every number in the range
// `[0, iterations-1]`. If the mapping function returns an error, iteration
// will stop prematurely and Run will return that error once all outstanding
// calls to the mapping function complete.
//
...
//   _does_ cause iteration to stop, the error returned from RunWithContext will
//   be the value of `parent.Err()`. The passed context may be nil, in which
//   case it defaults to `context.Background()`.
...
// files in parallel. Using Run, the first error would cause no _new_ files
// to begin downloading, but it still would not return until all of the in
